...
const FactorCompositeCreate: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const newFactor = {
      factorCode: 'COMPOSITE_' + Date.now(),
      factorName: name,
      factorCategory: '复合因子',
      factorType: '加权',
      description: '由多个基础因子加权生成复合因子',
      status: 'active',
  ...
  return (
    <div>
      ...
      {step === 1 && (
        <div>
          <Input placeholder="复合因子名称" value={name} onChange={e => setName(e.target.value)} style={{width: 200, marginBottom: 16}} />
          <Table
            rowKey="id"
            columns={[
              { title: '因子代码', dataIndex: 'factorCode' },
...
export default FactorCompositeCreate; 